...
- [SSTI Quote Filter Bypass via `__dict__.update()` (Example CTF)](#ssti-quote-filter-bypass-via-__dict__update-example-ctf)
- [Thymeleaf SpEL SSTI + Spring FileCopyUtils WAF Bypass (Example CTF)](#thymeleaf-spel-ssti--spring-filecopyutils-waf-bypass-example-ctf)
...
## SSTI Quote Filter Bypass via `__dict__.update()` (Example CTF)
...
## Thymeleaf SpEL SSTI + Spring FileCopyUtils WAF Bypass (Example CTF)

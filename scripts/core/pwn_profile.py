...
def strings_sample(path: Path, min_len: int = 8, limit: int = 200) -> dict:
...
def main() -> int:
    p = argparse.ArgumentParser(description="Quick pwn profile: file/checksec/readelf/ldd/strings.")
    ...
    payload = {
        ...
        "checksec": run_cmd(["checksec", "--file", str(binary)]),
        "readelf": run_cmd(["readelf", "-h", str(binary)]),
        "ldd": run_cmd(["ldd", str(binary)]),
        "strings": strings_sample(binary, min_len=8, limit=200),
    }

    json_path = out_dir / "pwn_profile.json"
    txt_path = out_dir / "pwn_profile.txt"
    ...
    lines = [
        ...
        "",
        "== checksec ==",
        payload["checksec"]["out"].strip(),
        payload["checksec"]["err"].strip(),
        "",
        "== readelf -h ==",
        ...
        "",
        "== ldd ==",
        payload["ldd"]["out"].strip(),
        payload["ldd"]["err"].strip(),
        "",
    ...
    print(f"[+] pwn_profile: {json_path}")
    print(f"[+] pwn_profile_txt: {txt_path}")
    return 0
